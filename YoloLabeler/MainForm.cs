...
public partial class MainForm : Form
{
    ...
    {
        _updatingList = true;
        var selected = imageCanvas.SelectedAnnotation;
        annotationList.Items.Clear();
        var indexToSelect = -1;
        foreach (var annotation in imageCanvas.Annotations)
        {
            var idx = annotationList.Items.Add(new AnnotationListItem(annotation, DisplayText(annotation)));
        ...
        annotationList.SelectedIndex = indexToSelect;
        _updatingList = false;
    }
    ...
    private void MainForm_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        const float step = 2f;
        if (!e.Control && imageCanvas.SelectedAnnotation != null)
        {
            var move = e.KeyCode switch
            {
            ...
            if (move != PointF.Empty)
            {
                imageCanvas.MoveSelection(move);
                RefreshAnnotationList();
                e.Handled = true;
                e.SuppressKeyPress = true;
            }
        }
        else if (e.Control && imageCanvas.SelectedAnnotation != null)
        {
            var resize = e.KeyCode switch
            {
                Keys.Left => new SizeF(-step, 0),
                Keys.Right => new SizeF(step, 0),
                Keys.Up => new SizeF(0, -step),
                Keys.Down => new SizeF(0, step),
                _ => SizeF.Empty
            };

            if (resize != SizeF.Empty)
            {
                imageCanvas.ResizeSelection(resize);
                RefreshAnnotationList();
                e.Handled = true;
                e.SuppressKeyPress = true;
            }
    ...
    private void annotationList_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (_updatingList)
        {
            return;
        ...
        {
            imageCanvas.SelectAnnotation(item.Annotation);
        }
    }

    private sealed class AnnotationListItem
    {
        public AnnotationListItem(Annotation annotation, string text)
        {
